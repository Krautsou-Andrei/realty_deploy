...
class MLSReal
{
  ...
  var $sql_card_tetx = 'select COMPID,CARDNUM,PICTURES,PICNAMES,
   (SELECT VOCCITY.NAME FROM VOCCITY WHERE VOCCITY.ID=COMMERC.CITYID) as Город,
   ...
   COORDN,
   (select PHONE from VOCAGENTS where ID=AGENTID and VOCAGENTS.COMPID=COMMERC.COMPID limit 1) as PHONE,
   (select CONCAT(NAME, " ",SIRNAME) from VOCAGENTS where ID=AGENTID and VOCAGENTS.COMPID=COMMERC.COMPID limit 1) as Агент,
   (select PHOTO from VOCAGENTS where ID=AGENTID and VOCAGENTS.COMPID=COMMERC.COMPID limit 1) as AGPHOTO,
   (select SGROUP from VOCAGENTS where ID=AGENTID and VOCAGENTS.COMPID=COMMERC.COMPID limit 1) as SGROUP,
   VTOUR,
   SQMPRICE,
   VARIANT,
   (SELECT VOCALLDATA.NAME FROM VOCALLDATA WHERE VOCALLDATA.ID=OBJID) as OBJ,
   (SELECT VOCALLDATA.NAME FROM VOCALLDATA WHERE VOCALLDATA.ID=USAGEID) as `USAGE`,
   DATECOR,
   PICCOUNT
   from COMMERC';
  ...
  function start()
  {
    ...
    if ($this->realty_type == 'stats') {
      ...
      $whereOD = '';
      if (isset($this->area_ot) && $this->area_ot != '') $whereOD .= ' SQUEAR>=' . $this->area_ot . ' AND ';
      if (isset($this->area_do) && $this->area_do != '') $whereOD .= ' SQUEAR<=' . $this->area_do . ' AND ';
      ...
      if ($whereS != '' || $whereM != '') {
        ...
        if ($whereM != '') {
          foreach (explode(',', $whereM) as $rn) $where .= 'OBJID in (' . $whereM . ') AND ';
        }
      ...
      $where .= " AND COMMERC.COMPID = " . $this->agency;
      $hav = '';
      $this->rc = mysqli_fetch_row(mysqli_query($this->dbcnx, 'select count(*) from COMMERC' . $where));
      $this->rc = $this->rc[0];
      ...
      // $sql_ht = mysqli_query($this->dbcnx,'select ID,NAME,CITYID from VOCALLDATA where RAZDELID=4');
      if ($this->filter == 1) {
        $sql_price = mysqli_query($this->dbcnx, 'select MIN(PRICE),MAX(PRICE) from COMMERC where ' . (isset($this->stype) && $this->stype == 1 ? 'VARIANT=2' : 'VARIANT<>2') . ' and COMPID=' . $this->agency);
        $dataPrice = mysqli_fetch_row($sql_price);
        $sql_area = mysqli_query($this->dbcnx, 'select MIN(SQUEAR),MAX(SQUEAR) from COMMERC where ' . (isset($this->stype) && $this->stype == 1 ? 'VARIANT=2' : 'VARIANT<>2') . ' and COMPID=' . $this->agency);
        $dataArea = mysqli_fetch_row($sql_area);
        $sql_earea = mysqli_query($this->dbcnx, 'select MIN(EAREA),MAX(EAREA) from COMMERC where ' . (isset($this->stype) && $this->stype == 1 ? 'VARIANT=2' : 'VARIANT<>2') . ' and COMPID=' . $this->agency);
...
<div class="realty_filter search_filter"><div class="col-xs-12 realty_filter__wrapper">
 	 ...
 	 <div class="col-xs-12">
					...
					<select class="form-control" name="razdel" onchange="javascript:location=this.value;">
                        ...
                        <option value="' . $this->pagelink4 . '" selected>Коммерческая</option>
                    </select>
	         </div>
...
<div class="row realty_filter search_filter"><div class="col-xs-12">
   ...
   <div class="op10 col-lg-3 col-md-4 col-sm-6 col-xs-12">
    ...
    echo '</select>
          ...
          <div class="op10 col-lg-3 col-md-4 col-sm-6 col-xs-12">
			<select id="mater" multiple name="mater[]" class="form-control" data-placeholder="Объект"></select>
           </div>
                 ...
                 var selectize=[];
						var $sel_tools2=$(\'#raions\').selectize({
							...
							plugins: [\'remove_button\']
			         		...
			         		var data_cel4=[{id:174,text:\'Офис\'},
  ...
  function createStats($stats, $rc)
  {
    ...
    echo '<div class="row">
		        <div class="col-xs-12">
			        <div class="catalog__subtitle">' . num_word($rc, array('Найден', 'Найдено', 'Найдено'), false) . ' ' . num_word($rc, array('объект', 'объекта', 'объектов')) . '</div>
		        </div>
		      </div>';
  ...
  {
    include "file.php";
    $sql_text_summ = 'select round(PRICE) AS PRICE, round(SQUEAR) AS AREA ';

    $where = ' WHERE COMPID=' . $agency . ' AND PICCOUNT>0 ';

    $dbcnx = mysqli_connect($dblocation, $dbuser, $dbpasswd, $dbname);
    if (!$dbcnx) exit();
    $sq_all_price = mysqli_query($dbcnx, $sql_text_summ . ' FROM COMMERC ' . $where . ' and VARIANT<>2 order by DATEINP DESC ');
    ...
    <h3>Кто-то его купил, или снял с продажи, но если вы посмотрите на сайте вы возможно найдете подходящий вариант.</h3>';
      ...
      $pTtl = ($cnt["VARIANT"] == 2 ? 'Сдаётся' : 'Продаётся') . ' ' . mb_strtolower($cnt["OBJ"]);
      $addr = $town . ", " . (trim($cnt["Район"]) != '' ? str_replace('р-н', '', $cnt["Район"]) . " р-н, " : '') . $cnt["Улица"];

      $pTtl1 = $town . ", " . mb_strtolower($cnt["OBJ"]) . ", на " . $cnt["Улица"] . ", " . $cnt["Этаж"] . "-'этажный, " . $cnt["Материал стен"] . ",площадь: " . $cnt["SQUEAR"] . " кв.м, цена: " . number_format($cnt["Цена общая"] / 1000, 3, ' ', ' ') . " рублей.";
...
<div class="product__gallery">
      ...
      if ($cnt["SQUEAR"] > 0) echo '<span>' . number_format((($cnt["Цена общая"] / $cnt["SQUEAR"]) / 1000), 3, ' ', ' ') . ' ₽/м²</span>';
      echo '
						 </div>						 
                      <div class="second-product-info__label">' . (!empty($average_price) && $average_price >= intval($cnt["Цена общая"] / $cnt["SQUEAR"]) ? "Хорошая цена!" : "Элитная недвижимость!")  . '</div>
                    </div>
                    <div class="second-product-info__list">
                      <div class="second-product-info__item">
                                ...
                                <div class="info">
                                  ...
                                  <span class="value">' . $cnt["SQUEAR"] . ' м²</span>
                                </div>
                              </div>                                                                                                           </div>
                  </div>
                </div>
                  ...
                  <section class="more-list">
                    <div class="more-list__column more-column">
                      <label class="more-tabs__label">
                        ...
                        <span class="more-column__title title--lg">Об объекте</span>
                      </label>
                      <div class="more-column__list">';
      if ($cnt["OBJ"] != '') echo '<div class="more-column__row"><span>Тип объекта</span><span>' . mb_strtolower($cnt["OBJ"]) . '</span></div>';
      if ($cnt["USAGE"] != '') echo '<div class="more-column__row"><span>Использование</span><span>' . mb_strtolower($cnt["USAGE"]) . '</span></div>';

      if ($cnt["SQUEAR"] > 0) echo '<div class="more-column__row"><span>Общая площадь</span><span>' . $cnt["SQUEAR"] . ' м²</span></div>';
      if ($cnt["EAREA"] > 0) echo '<div class="more-column__row"><span>Площадь участка</span><span>' . $cnt["EAREA"] . ' сот.</span></div>';
      ...
      echo '	  
              ...
              <div class="single-page__order">
                <article class="agent-order" data-agent-order="">
                    ...
                    <div class="agent-order__label">' . (!empty($average_price) && $average_price >= intval($cnt["Цена общая"] / $cnt["SQUEAR"]) ? "Хорошая цена!" : "Элитная недвижимость!")  . '</div>
      ...
      if ($cnt["SQUEAR"] > 0) echo '<span class="agent-price-one-mert__price">' . number_format((($cnt["Цена общая"] / $cnt["SQUEAR"]) / 1000), 3, ' ', ' ') . ' ₽/м²</span>';
      echo '
                    </div>
                    <div class="button-wrapper">
                        <div class="agent-order__button">
                            ...
                            <button class="button--favorites-mobile ' . ($is_favorite ? 'delete' : '') . '" type="button" data-favorite-cookies="' . $cnt['CARDNUM'] . '" data-category-cookie="COMMERC" data-button-favorite-mobile data-delete-favorite="' . $is_favorite . '"><span></span></button>
                        </div>
                        <div class="agent-order__callback" onclick="setLink(event,\'' . esc_url('?obj=' . $cnt['CARDNUM']) . '\',\'kommercheskaya\')">
                        ...
                        <div class="agent-order__favorites">
                            <button class="button button--favorites" type="button" data-favorite-cookies="' . $cnt['CARDNUM'] . '" data-category-cookie="COMMERC" data-button-favorite data-delete-favorite="' . $is_favorite . '">
                                <span>';
      if ($is_favorite) {
        echo "удалить";
      } else {
