...
class MLSReal
{
  ...
  var $cardSale    = 'Продажа участка';
  var $cardRent    = 'Аренда участка';
  var $mlspage     = 1;
  var $gets        = '';
  var $order       = 1;
...
*/
      ...
      if ($whereS != '' || $whereM != '') {
        ...
        if ($whereM != '') {
          foreach (explode(',', $whereM) as $rn) $where .= 'USAGEID in (' . $whereM . ') AND ';
        }
      ...
      $hav = '';
      //if(trim($this->streetsearch)!='') $hav=' and STREETID IN (SELECT ID FROM VOCSTREET WHERE NAME like "%'.trim($this->streetsearch).'%") ';
      ...
      if ($this->filter == 1) {
        $sql_price = mysqli_query($this->dbcnx, 'select MIN(PRICE),MAX(PRICE) from HOUSES where ' . (isset($this->stype) && $this->stype == 1 ? 'VARIANT=2' : 'VARIANT<>2') . ' and WHAT=1 and COMPID=' . $this->agency);
        $dataPrice = mysqli_fetch_row($sql_price);
        $sql_earea = mysqli_query($this->dbcnx, 'select MIN(EAREA),MAX(EAREA) from HOUSES where ' . (isset($this->stype) && $this->stype == 1 ? 'VARIANT=2' : 'VARIANT<>2') . ' and WHAT=1 and COMPID=' . $this->agency);
    ...
    switch ($this->realty_type) {
      case 'stats':
        $this->echofilter($sql_city, $sql_raion, $whereS, $whereM, $content, $contentMAP, round($dataPrice[0]), round($dataPrice[1]), round($dataEArea[0]), round($dataEArea[1]));
  ...
  function echofilter($sql_city, $sql_raion, $whereS, $whereM, $stats, $statsMAP, $dataPriceMIN, $dataPriceMAX, $dataEAreaMIN, $dataEAreaMAX)
  {
    $echo_city = '';
...
<div class="realty_filter search_filter"><div class="col-xs-12">
 	 ...
 	 <div class="col-xs-12">
					...
					<select class="form-control" name="razdel" onchange="javascript:location=this.value;">
                        ...
                        <option value="' . $this->pagelink2_1 . '" selected>Участки</option>
   ...
   <div class="mt1 col-xs-12">
    ...
    <div class="ui-slider-title mt1 col-xs-12">
...
<div class="row realty_filter search_filter"><div class="col-xs-12 realty_filter__wrapper">
   ...
   <div class="op10 col-lg-3 col-md-4 col-sm-6 col-xs-12">
    ...
    echo '</select>
		   ...
		   <div class="op10 col-lg-3 col-md-4 col-sm-6 col-xs-12">
			<select id="mater" multiple name="mater[]" class="form-control" data-placeholder="Категория земель"></select>
           </div>
		...
		$( "#slider-mlsEArea" ).slider({
		  ...
		  slide: function( event, ui ) {
			...
			$( "#mlsEArea" ).html( " от " + ui.values[ 0 ] + " до " + ui.values[ 1 ]  );
             ...
             <div class="op10 col-lg-push-4 col-lg-2 col-md-8 col-sm-6 col-xs-12 text-right pt25">
                 ...
                 var selectize=[];
						var $sel_tools2=$(\'#raions\').selectize({
							...
							plugins: [\'remove_button\']
			         		...
			         		var data_cel4=[{id:134,text:\'населённых пунктов\'},
											{id:137,text:\'сельхозназначения\'},
											{id:135,text:\'промышленности\'},
											{id:138,text:\'лесного фонда\'},
											{id:139,text:\'водного фонда\'},
											{id:144,text:\'особо охраняемых территорий и объектов\'},
											{id:143,text:\'запаса\'}
											];
  ...
  function createStats($stats, $rc)
  {
    ...
    echo '<div class="row">
		        <div class="col-xs-12">
			        <div class="catalog__subtitle">' . num_word($rc, array('Найден', 'Найдено', 'Найдено'), false) . ' ' . num_word($rc, array('участок', 'участка', 'участков')) . '</div>
		        </div>
		      </div>';
  ...
  {
    include "file.php";
    $sql_text_summ = 'select PRICE, EAREA';

    $where = ' WHERE COMPID=' . $agency . ' AND PICCOUNT>0 ';

    $dbcnx = mysqli_connect($dblocation, $dbuser, $dbpasswd, $dbname);
    if (!$dbcnx) exit();
    $sq_all_price = mysqli_query($dbcnx, $sql_text_summ . ' FROM HOUSES ' . $where . ' and WHAT=1 and VARIANT<>2 order by DATEINP DESC ');
    ...
    if (!empty($sq_all_price)) {
      if (mysqli_num_rows($sq_all_price) > 0) {
        while ($price = mysqli_fetch_array($sq_all_price)) {
          if ($price['EAREA'] > 0) {
            $sq_int_sq_price_array[] = intval(intval($price['PRICE']) / $price['EAREA']);
          }
        }
      }
    }
    ...
    <h3>Кто-то его купил, или снял с продажи, но если вы посмотрите на сайте вы возможно найдете подходящий вариант.</h3>';
      ...
      $pTtl = ($cnt["VARIANT"] == 2 ? 'Сдаётся' : 'Продаётся') . ' участок ' . $cnt["EAREA"] . '<small> сот.</small>';
      $addr = $town . ", " . (trim($cnt["Район"]) != '' ? str_replace('р-н', '', $cnt["Район"]) . " р-н, " : '') . $cnt["Улица"];

      $pTtl1 = $town . ", участок, на " . $cnt["Улица"] . ", площадью: " . $cnt["EAREA"] . " сот., цена: " . number_format($cnt["Цена общая"] / 1000, 3, ' ', ' ') . " рублей.";
...
<div class="product__gallery">
      ...
      if ($cnt["EAREA"] > 0) echo '<span class="agent-price-one-mert__price">' . number_format((($cnt["Цена общая"] / $cnt["EAREA"]) / 1000), 3, ' ', ' ') . ' ₽/сот</span>';
      echo '    
						 </div>
                      <div class="second-product-info__label">' . (!empty($average_price) && $average_price >= intval($cnt["Цена общая"] / $cnt["EAREA"]) ? "Хорошая цена!" : "Элитная недвижимость!")  . '</div>
                    </div>
                    <div class="second-product-info__list">
                      <div class="second-product-info__item">
                                ...
                                <div class="info">
                                  ...
                                  <span class="value">' . $cnt["EAREA"] . ' сот</span>
                                </div>
                              </div>                     
                                                                                     </div>
                  </div>
      ...
      echo '
              ...
              <div class="single-page__order">
                <article class="agent-order" data-agent-order="">
                  ...
                  <div class="agent-order__label">' . (!empty($average_price) && $average_price >= intval($cnt["Цена общая"] / $cnt["EAREA"]) ? "Хорошая цена!" : "Элитная недвижимость!")  . '</div>
      ...
      echo '
                    ...
                    <div class="button-wrapper">
                        ...
                        <div class="agent-order__favorites">
                            <button class="button button--favorites" type="button" data-favorite-cookies="' . $cnt['CARDNUM'] . '" data-category-cookie="HOUSES" data-category-type-cookie="UCHASTOK" data-button-favorite data-delete-favorite="' . $is_favorite . '">
                            <span>';
      if ($is_favorite) {
        echo "удалить";
      } else {
